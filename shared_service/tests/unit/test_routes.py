...
from models import User  # Added Log for register tests
from routes import get_db, log_error, log_request
...
def test_register_user_already_exists(client):
    ...
    mock_db_session.commit = AsyncMock(
        side_effect=IntegrityError("mock error", params={}, orig=None)
    )
...
def test_login_success(client):
    ...
    with patch(
        "routes.verify_credentials", AsyncMock(return_value=mock_user)
    ) as _:
        response = client.post("/api/v1/auth/login", auth=("testuser", "testpassword"))
...
def test_login_invalid_credentials(client):
    ...
    with patch(
        ...
        response = client.post(
            "/api/v1/auth/login", auth=("wronguser", "wrongpassword")
        )
