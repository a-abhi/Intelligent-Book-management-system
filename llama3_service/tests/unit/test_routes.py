...
from schemas import BookSummaryResponse, ReviewSummaryResponse
...
def test_generate_review_summary_success():
    ...
    mock_llama_response = {
        "response": "This is a mock summary of the reviews."
    }

    # 4. Mock httpx.AsyncClient
    with patch('httpx.AsyncClient.post', AsyncMock(return_value=MagicMock(
        status_code=200,
        json=lambda: mock_llama_response
    ))) as mock_post, \
...
def test_generate_review_summary_llama_error():
    ...
    with patch('httpx.AsyncClient.post', AsyncMock(side_effect=Exception("LLaMA3 API error"))) as mock_post, \
    patch('routes.log_action', AsyncMock()) as mock_log_action:
